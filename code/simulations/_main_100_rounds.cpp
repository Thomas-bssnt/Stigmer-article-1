...
int main()
{
    ...
    for (auto &gameType : gameTypes)
    {
        ...
        const std::string pathObservables{pathDataFigures + "pred_100_rounds/" + gameType + "/"};
...
#pragma omp parallel for
        for (int iGame = 0; iGame < numberOfGames; ++iGame)
        {
            ...
            std::vector<Player> players{initializePlayers(parametersPlayersType, initializationType, game,
                                                          parametersVisits, parametersStars)};
        ...
        // N_def n'est plus défini
        // getS_team_N_def(S_team, N_def, mean, median);
        // saveObservable(pathObservables + "S_team_N_def_mean", mean);
        // saveObservable(pathObservables + "S_team_N_def_median", median);
    }
    return 0;
}
